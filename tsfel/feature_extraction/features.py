...
def mean_frequency(signal, fs):
    """Computes mean frequency of the signal.
        ...
        ----------
        signal : nd-array
            Input from which mean frequency is computed
        fs : int
            Sampling frequency
        ...
        -------
        f_mean : float
           Weighted average of the frequencies, weights being the amplitude values.
        """
    f, fmag = calc_fft(signal, fs)

    # Calculate power at each frequency component (square of the amplitude)
    power = fmag ** 2

    # Compute the mean frequency (spectral centroid)
    total_power = np.sum(power)
    if total_power == 0:
        return 0  # Prevent division by zero
    f_mean = np.sum(f * power) / total_power

    return f_mean


@set_domain("domain", "spectral")
def std_frequency(signal, fs):
    """Computes standard deviation of the frequencies of the signal.

        Parameters
        ----------
        signal : nd-array
            Input from which standard deviation frequency is computed
        fs : int
            Sampling frequency
        ...
        -------
        f_std : float
           Weighted standard deviation of the frequencies, weights being the amplitude values.
        """
    f, fmag = calc_fft(signal, fs)
    ...
    f_variance = np.sum(((f - f_mean) ** 2) * power) / total_power

    # Standard deviation is the square root of the variance
    f_std = np.sqrt(f_variance)
