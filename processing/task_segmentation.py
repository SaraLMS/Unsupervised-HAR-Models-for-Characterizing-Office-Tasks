...
def _validate_walking_starts_stops(yacc, folder_name, starts, stops, min_value_between_starts: int,
                                   min_stop_value: int):
    ...
    for i in range(len(starts) - 1):
        ...
        if starts[i + 1] - starts[i] >= min_value_between_starts:
            valid_starts.append(starts[i])

    # append the last element since it has no next element to compare
    valid_starts.append(starts[-1])

    # validade the stop indices

    # Find indices of elements that are below 30000
    valid_stops = stops[stops >= min_stop_value].tolist()
